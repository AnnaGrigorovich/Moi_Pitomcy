...
def test_successful_update_self_pet_info(name='Мурзик', animal_type='Котэ', age=5):
    ...
    def test_add_new_pet_without_photo(name='Хомяк', animal_type='неизвестен', age='1'):
        """Проверяем возможность добавления питомца без фото с корректными данными"""

        _, auth_key = pf.get_api_key ( valid_email, valid_password )

        # Запрос на добавление нового питомца без фото, который созраняем в переменную result, а в переменныю status сохраняем статус ответа на этот запрос
        status, result = pf.add_new_pet_without_photo ( auth_key, name, animal_type, age )

        assert status == 200  # вызываем проверку статуса ответа, который должен быть равен 200
        assert result[
                   'name' ] == 'Хомяк'  # Вызываем проверку того что в переменной result имя питомца совпадает с тем именем питомца,которое мы указали при добавлении(name= 'Хомяк')

    def test_add_photo_of_pets_valid(pet_photo='images/homyk.jpg'):
        ...
        if len ( list_my_pets[ 'pets' ] ) == 0:
            pf.add__about_new_pet ( auth_key, 'Pol', 'ОН', '1', 'images/homyk.jpg' )
            _, list_my_pets = pf.get_list_of_pets ( auth_key, 'my_pets' )  # Запрашиваем список питомцев

        pet_id = list_my_pets[ 'pets' ][ 0 ][
            'id' ]  # Создаем перемнную pet_id в которую сохраняем id первого в списке питомца